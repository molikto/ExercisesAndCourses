...
word_embedding_weights = init_wt * randn(vocab_size, numhid1); % a word has 50 feature
embed_to_hid_weights = init_wt * randn(numwords * numhid1, numhid2); % 3 * 50 feature for 3 words. and get 200 feature for the piece
hid_to_output_weights = init_wt * randn(numhid2, vocab_size); % and then from the info of the piece to the info of next one!
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    [embedding_layer_state, hidden_layer_state, output_layer_state] = ... % is just break the line... shit
    ...
    % MEASURE LOSS FUNCTION. - it is only for printing!!! and the ce is
    % printed every something... just this!
