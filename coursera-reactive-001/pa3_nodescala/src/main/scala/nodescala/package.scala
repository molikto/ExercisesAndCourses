...
package object nodescala {
   ...
   */
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyRef {
   ...
   */
  implicit class FutureOps[T](val f: Future[T]) extends AnyRef {
     ...
     */
    def now: T = {
      if (f.isCompleted) Await.result(f, 1 milli  ) else throw new NoSuchElementException
    }
  ...
  object CancellationTokenSource {
     ...
     */
    def apply(): CancellationTokenSource = new CancellationTokenSource with CancellationToken {
      def cancellationToken: CancellationToken = this
      val p = promise[Unit]
      def isCancelled = p.isCompleted
      def unsubscribe(): Unit = p success Unit
    }
  }
