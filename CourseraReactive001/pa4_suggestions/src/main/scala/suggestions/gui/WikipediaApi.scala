...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      Observable(observer => {
        val s = obs.materialize.subscribe(
          t =>  t.asJava.getKind match {
            case Kind.OnNext => observer.onNext(Success(t.asJava.getValue))
            case Kind.OnError => observer.onNext(Failure(t.asJava.getThrowable))
              observer.onCompleted()
            case Kind.OnCompleted => observer.onCompleted()
    ...
    def timedOut(totalSec: Long): Observable[T] =
      obs.combineLatest(Observable(-1) ++ Observable.interval(totalSec seconds)).takeWhile(_._2 == -1).map(_._1)
