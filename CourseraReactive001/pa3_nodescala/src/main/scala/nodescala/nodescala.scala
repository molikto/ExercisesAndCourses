...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val c1 = l.start()
    val c2 = CancellationTokenSource()
    def o(a: Request, b: Exchange): Unit =
        if (c2.cancellationToken.nonCancelled) {
          respond(b, c2.cancellationToken ,handler(a))
          if (c2.cancellationToken.nonCancelled)
            reg()
        }
    def reg() =
      l.nextRequest() onSuccess {
        case (a: Request, b: Exchange) => {
          o(a, b)
        }
      }
    reg()
